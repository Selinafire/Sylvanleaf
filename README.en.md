...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/Scrapyard/wiki/Home) | [English Home](https://github.com/alkaid616/Scrapyard/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `Scrapyard`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `Scrapyard.json` and update `id`, `name`, `author`, and `description`.
3. Edit `ScrapyardCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `Scrapyard/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
>
> `dependencies[STS2-RitsuLib].version` in `Scrapyard.json` **must exactly match** the `STS2.RitsuLib` version your `.csproj` actually compiles against. The two are independent and **never auto-synced** — mismatches let players pass the manifest check and crash at runtime, or get wrongly rejected when their RitsuLib would have worked. See [Pre-release checklist: version alignment](#pre-release-checklist-version-alignment) below for the step-by-step procedure.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `Scrapyard.json` is what the game loader checks at runtime. The two are independent and never auto-synced.**
...
2. Write that version into `dependencies[STS2-RitsuLib].version` in `Scrapyard.json`.
...
| Command | Behavior |
|---|---|
| `dotnet build .\Scrapyard.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/Scrapyard` directory.
...
```text
Scrapyard/
├── ScrapyardCode/   # C# source
├── Scrapyard/       # Godot resources, localization, and placeholder scenes
├── Scrapyard.csproj
├── Scrapyard.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://Scrapyard/...` is the Godot/PCK resource path, mapping to the repository resource directory `Scrapyard/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Property | Value |
|---|---|
| Type | `ScrapyardCharacter` |
| Expected ID | `SCRAPYARD_CHARACTER_SCRAPYARD_CHARACTER` |
| Starter deck | 4 × `ScrapyardStrike`, 4 × `ScrapyardDefend`, 1 × `ScrapyardRelic` |
...
|---|---|---|
| `ScrapyardStrike` (attack) | character card pool | `SCRAPYARD_CARD_SCRAPYARD_STRIKE` |
| `ScrapyardDefend` (skill) | character card pool | `SCRAPYARD_CARD_SCRAPYARD_DEFEND` |
| `ScrapyardRelic` | `ScrapyardRelicPool` | `SCRAPYARD_RELIC_SCRAPYARD_RELIC` |

### Static placeholder assets

**Images** (`res://Scrapyard/images/...`):

- `cards/ScrapyardStrike.png`, `cards/ScrapyardDefend.png`: sample card art.
- `relics/ScrapyardRelic.png`: sample relic icon.
- `characters/Scrapyard_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://Scrapyard/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `Scrapyard_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `Scrapyard_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `Scrapyard_merchant.tscn` | Merchant | — |
| `Scrapyard_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `Scrapyard_character_select_bg.tscn` | Character select background | — |
...
`Scrapyard.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
